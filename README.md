...
Заполнение матрицы происходит с помощью метода fill_Matrix. При помощи вложенного цикла считываются числа и сохраняются в матрицу.
...
```
Для такой сортировки реализован метод criteria_Sort которая принимает на входе матрицу и сортирует ее. 
...
После того как статистика по каждой строке собрана, данные в массиве analytics сортируются пузырьком в методе sort_Matrix. Сначала сортируются первые значения, при их совпадении сортируются вторые и так далее.
...
Для поворота массива на 90 градусов написан метод rotate_Matrix который возвращает новую двумерную матрицу, повернутую на 90 градусов. Рассмотрим новые координаты каждого элемента:
